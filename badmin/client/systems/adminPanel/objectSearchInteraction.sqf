...
if (_uid call isAdmin) then
{
	...
	switch (_switch) do
	{
	    case OBJECT_SEARCH_ACTION_FIND:
		{
			...
			//diag_log format["[A3BG_LOG] Search class is %1", _objectClass];
...

			{
				...
				//diag_log format["[A3BG_LOG] Setting data to %1", str(_objPos)];
		...
		case OBJECT_SEARCH_ACTION_TELEPORT:
		{
			...
			diag_log format["[A3BG_LOG] _objPos is %1", _objPos];
